...
- 只有方阵才可能有逆矩阵
- 逆矩阵若存在，则唯一
- 对方阵而言，只要 $B$ 满足一侧条件（如 $BA=I$），它就自动满足另一侧（$AB=I$），即单侧逆即为双侧逆
...
#### 5.3.2 一般 2×2 分块矩阵（舒尔补）

```math
A = \begin{pmatrix} A_{11} & A_{12} \\ A_{21} & A_{22} \end{pmatrix}
```

当 $A_{11}$ 可逆时，记**舒尔补** $S = A_{22} - A_{21}A_{11}^{-1}A_{12}$。此时 $\det(A) = \det(A_{11})\det(S)$，故 $A$ 可逆当且仅当 $S$ 可逆；可逆时逆矩阵可借助舒尔补 $S$ 写出。

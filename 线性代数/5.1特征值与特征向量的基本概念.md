...
> **性质3.3（正交矩阵的特征值）**
>
> （实）正交矩阵的特征值可能为复数（如旋转矩阵的特征值为 $\pm i$），但其模长均为1，即 $|\lambda| = 1$。
...
> 2. **线性封闭性（子空间结构）**：属于同一特征值 $\lambda$ 的特征向量，其任意线性组合若非零，仍是 $\lambda$ 的特征向量（即特征子空间 $V_\lambda$ 对线性运算封闭）。
...
**例题6.1**：求矩阵 $A = \begin{pmatrix} 3 & 1 & 0 \\ 1 & 3 & 0 \\ 0 & 0 & 2 \end{pmatrix}$ 的特征值和特征向量。
...
**例题6.2**：设 $A = \begin{pmatrix} 1 & a & 0 \\ 0 & 2 & 0 \\ 0 & 0 & 3 \end{pmatrix}$，若 $A$ 有特征值 $\lambda = 2$，求参数 $a$ 的值。
...
**例题6.3**：设 $A$ 是3阶矩阵，特征值为 $1, 2, 3$，求 $A^2 + 2A + 3I$ 的特征值。
...
**例题6.4**：设 $A = \begin{pmatrix} 2 & 1 & 1 \\ 1 & 2 & 1 \\ 1 & 1 & 2 \end{pmatrix}$，求 $A^{10}$ 的特征值。
...
```math
= (\lambda-4)[(\lambda-3)^2 + 4(\lambda-3) + 4] = (\lambda-4)(\lambda-1)^2
```

化简得特征多项式为 $(\lambda-4)(\lambda-1)^2$。

**利用矩阵结构验证**：$A = I + J$，其中 $J$ 是全1矩阵。$J$ 的特征值为 $3$（对应 $(1,1,1)^T$，因为 $J(1,1,1)^T = 3(1,1,1)^T$）和 $0$（二重），故 $A$ 的特征值为 $1+3=4$ 和 $1+0=1$（二重）。

验证：$\text{tr}(A) = 4+1+1 = 6$，$\det(A) = 4 \times 1 \times 1 = 4$，与 $\det(A) = 2(4-1) - 1(2-1) + 1(1-2) = 4$ 一致。

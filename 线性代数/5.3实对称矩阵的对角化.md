...
**几何意义**：
- 矩阵 $A$ 可表示为以特征值 $\lambda_i$ 为系数、以秩1投影矩阵 $q_i q_i^T$ 为基底的线性组合
...
**步骤1**：求特征值
```math
\det(\lambda I - A) = \det\begin{pmatrix} \lambda-1 & -1 & 0 \\ -1 & \lambda-1 & 0 \\ 0 & 0 & \lambda-2 \end{pmatrix} = (\lambda-2)[(\lambda-1)^2 - 1] = (\lambda-2)\cdot\lambda(\lambda-2) = \lambda(\lambda-2)(\lambda-2) = \lambda(\lambda-2)^2
```
...
对于 $\lambda_1 = 0$：
```math
(0 \cdot I - A)\xi = 0 \Rightarrow \begin{pmatrix} -1 & -1 & 0 \\ -1 & -1 & 0 \\ 0 & 0 & -2 \end{pmatrix}\xi = 0
```

解得：$x_1 + x_2 = 0$，$x_3 = 0$

特征向量：$\xi_1 = \begin{pmatrix} 1 \\ -1 \\ 0 \end{pmatrix}$

对于 $\lambda_2 = \lambda_3 = 2$：
```math
(2I - A)\xi = 0 \Rightarrow \begin{pmatrix} 1 & -1 & 0 \\ -1 & 1 & 0 \\ 0 & 0 & 0 \end{pmatrix}\xi = 0
```

解得：$x_1 = x_2$，$x_3$ 自由

特征向量：$\xi_2 = \begin{pmatrix} 1 \\ 1 \\ 0 \end{pmatrix}$，$\xi_3 = \begin{pmatrix} 0 \\ 0 \\ 1 \end{pmatrix}$

**步骤3**：验证正交性
$\xi_1 \cdot \xi_2 = 1 \cdot 1 + (-1) \cdot 1 + 0 \cdot 0 = 0$，$\xi_1 \cdot \xi_3 = 0$，$\xi_2 \cdot \xi_3 = 0$
...
**答案**：
正交矩阵 $Q = \begin{pmatrix} \frac{1}{\sqrt{2}} & \frac{1}{\sqrt{2}} & 0 \\ -\frac{1}{\sqrt{2}} & \frac{1}{\sqrt{2}} & 0 \\ 0 & 0 & 1 \end{pmatrix}$，对角矩阵 $\Lambda = \begin{pmatrix} 0 & 0 & 0 \\ 0 & 2 & 0 \\ 0 & 0 & 2 \end{pmatrix}$

### 4.2 重特征值情况的处理

当特征值有重数时，需要对对应特征子空间中的特征向量进行正交化。

> **例题4.2**：将矩阵 $B = \begin{pmatrix} 3 & -2 & -2 \\ -2 & 6 & -1 \\ -2 & -1 & 6 \end{pmatrix}$ 正交对角化。

**解答**：

**步骤1**：求特征值（计算过程略）
特征值：$\lambda_1 = 1$，$\lambda_2 = \lambda_3 = 7$

**步骤2**：求特征向量

对于 $\lambda_1 = 1$：特征向量 $\alpha_1 = \begin{pmatrix} 2 \\ 1 \\ 1 \end{pmatrix}$

对于 $\lambda_2 = \lambda_3 = 7$：特征向量 $\alpha_2 = \begin{pmatrix} 1 \\ -2 \\ 0 \end{pmatrix}$，$\alpha_3 = \begin{pmatrix} 1 \\ 0 \\ -2 \end{pmatrix}$
...
**应用**：椭圆、椭球面、双曲面等二次曲线（面）的主轴方向，由对应二次型矩阵的特征向量确定。
...
**例题6.1**：求实对称矩阵 $A = \begin{pmatrix} 2 & -1 & -1 \\ -1 & 2 & -1 \\ -1 & -1 & 2 \end{pmatrix}$ 的正交对角化。
...
**例题6.2**：对向量组 $\alpha_1 = \begin{pmatrix} 1 \\ 1 \\ 1 \end{pmatrix}$，$\alpha_2 = \begin{pmatrix} 1 \\ 1 \\ 0 \end{pmatrix}$，$\alpha_3 = \begin{pmatrix} 1 \\ 0 \\ 0 \end{pmatrix}$ 进行Schmidt正交化。

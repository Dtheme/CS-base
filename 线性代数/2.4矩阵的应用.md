...
> **定理3.1（稳态分布）**
> 
> 对于有限状态的不可约马尔可夫链，存在唯一的稳态分布 $\boldsymbol{\pi}$，满足：
...
> **定理4.1（路径计数定理）**
> 
> 设 $A$ 是图的邻接矩阵，则 $A^k$ 的第 $(i,j)$ 元素表示从顶点 $i$ 到顶点 $j$ 长度为 $k$ 的途径（walk，即允许重复顶点和重复边的行走）的数目。
...
> **定理5.1（主成分分析）**
> 
> 对于数据矩阵 $X$（每行为一个样本、每列为一个变量），先对各变量中心化（必要时标准化），并据此计算样本协方差矩阵 $\Sigma$；则各主成分方向即为 $\Sigma$ 的特征向量，对应的特征值表示数据在该主成分方向上的方差。将特征值按从大到小排序，依次对应第一、第二……主成分。
...
- 从第1行：$x_1 + 2x_2 + 3x_3 = 14$，得 $x_1 = \frac{7}{6}$

因此解为：$x_1 = \frac{7}{6}$，$x_2 = \frac{8}{3}$，$x_3 = \frac{5}{2}$。
...
计算总产出：
```math
\mathbf{x} = (I - A)^{-1}\mathbf{y} = \begin{pmatrix} 1.5 & 0.5 \\ \frac{2}{3} & \frac{4}{3} \end{pmatrix} \begin{pmatrix} 100 \\ 200 \end{pmatrix} = \begin{pmatrix} 250 \\ \frac{1000}{3} \end{pmatrix}
```

因此，第一部门的总产出为 $250$，第二部门的总产出为 $\frac{1000}{3} \approx 333.33$。

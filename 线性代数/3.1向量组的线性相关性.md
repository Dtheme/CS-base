...
> 3. **成比例向量**：如果向量组中存在两个不同向量成比例，即 $\alpha_i = k\alpha_j\ (i\neq j)$，则向量组线性相关
...
> 3. **扩充性质**：线性无关组中加入一个向量后，新向量组线性相关的充要条件是该向量可由原向量组线性表示；否则新向量组仍线性无关
...
- 如果向量组中某个向量可由其余向量线性表示，则去掉该向量后，所得向量组与原向量组等价（秩不变，且原向量组中任一向量仍可由新向量组线性表示）。
...
**求极大线性无关组的步骤**：
1. 将向量组作为矩阵的列向量
2. 对矩阵进行初等行变换化为行阶梯形（即可，无需进一步化到行最简形）
3. 行阶梯形中主元（每行第一个非零元）所在的列，对应的原向量构成极大线性无关组（若还需写出其余向量的线性表示式，可继续化为行最简形后直接读出）
...
行阶梯形中主元位于第1列和第2列，所以 $\alpha_1, \alpha_2$ 构成一个极大线性无关组。
